...
class VectorStore:
    ...
    def initialize_faiss_store(self, texts: List[str], embeddings = List[str]) -> FAISS:
        ...
        if self.logger:
            self.logger.info(f"Initializing FAISS with {len(texts)} documents using embeddings: {self.embeddings}.")
        ...
        try:
            ...
            if self.logger:
                self.logger.info("FAISS vector store initialized and saved successfully!")

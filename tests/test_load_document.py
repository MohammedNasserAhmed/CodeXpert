...
def mock_loader_config():
    return DocumentLoaderConfig(
        root_dir="/dir/path", file_types=[".py"], recursive=True
    )
...
def test_load_documents(mock_loader):
    ...
    with patch(
        "codexpert.components.load_document.DocumentLoader.load_documents",
...
def test_save_documents(mock_loader):
    ...
    with patch(
        "codexpert.components.load_document.DocumentLoader.save_documents"
    ) as mock_save:

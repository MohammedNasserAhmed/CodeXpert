...
def test_load_documents(mock_loader):
    ...
    with patch("codeexp.components.load_document.DocumentLoader.load_documents", return_value=["doc1", "doc2"]) as mock_load:
...
def test_save_documents(mock_loader):
    ...
    with patch("codeexp.components.load_document.DocumentLoader.save_documents") as mock_save:
